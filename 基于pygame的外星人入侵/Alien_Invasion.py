        ...
        for energy in self.energy.copy(): # 检测飞船与能量球的碰撞
            if energy.rect.colliderect(self.ship): 
                ...
                if energy.image_prep == 'images/energy.png':
                    ...
                    self.energy.remove(energy)
        if not self.energy: # 如果能量球群为空
            m_time1 = self.energy.create_time # 获取之前创建能量球的时间
            m_time2 = time() # 获取当前时间
            # print(m_time2 - m_time1)
            if m_time2 - m_time1 > 5: # 如果距离上一次创建能量球的时间超过15秒
